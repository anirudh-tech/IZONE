  ...
  return (
    <>
      ...
      >
        ...
        {/* Chat bubble with message - only show if showBubble is true */}
        {showBubble && (
          <div
            className={`
              absolute bottom-16 rounded-2xl p-4 shadow-xl max-w-xs transform
              ${position === 'bottom-right' ? 'right-0' : 'left-0'}
              opacity-100 translate-y-0 transition-all duration-300
            `}
            style={{ 
              background: 'linear-gradient(135deg, #25d366 0%, #20b658 100%)'
            }}
          >
            {/* Close button */}
            <button
              onClick={handleCloseBubble}
              className="absolute -top-2 -right-2 bg-white text-gray-500 hover:text-gray-700 rounded-full w-6 h-6 flex items-center justify-center shadow-md transition-colors duration-200"
            ...
            <div 
              className={`
                absolute top-full w-0 h-0 border-t-[12px] border-t-green-500
                ${position === 'bottom-right' 
                  ? 'right-6 border-l-[12px] border-l-transparent' 
                  : 'left-6 border-r-[12px] border-r-transparent'
                }
              `}
            ></div>
